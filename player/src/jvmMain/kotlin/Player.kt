...
class Track(val trackNumber : Int) {
  ...
  fun schedulePattern(
    ...
    try {
      ...
      while (!isDone(patternEvent, iteration) &&
        ...
        val latch = engine().scheduleEvent(patternSchedule, patternName)
        ...
        immediateEvents.forEach { engine().schedule(it) }
        ...
        // a no-op.)
        if (engine().isPlaying) { engine().startSequencer() }
        ...
        if (!iterationEvents.isEmpty())
          startOffset = iterationEvents.map { endOffset(it) }.maxOrNull()!!
  ...
  fun scheduleEvents(events : List<Event>, _startOffset : Int) : Int {
    ...
    // FIXME: Review this older code from the kotlin-js branch and incorporate
    // it, if needed.
    //
    ...
    return scheduledEvents.map { endOffset(it) }.maxOrNull()!!
  }
...
private fun applyUpdates(updates : Updates) {
  ...
  updates.systemEvents.filter { it["type"] == "set-offset" }.forEach {
    awaitActiveTasks()
    engine().setSequencerOffset(it["offset"] as Int)
  }
  ...
  updates.systemEvents.filter { it["type"] == "tempo" }.forEach {
    engine().setTempo(it["offset"] as Int, it["bpm"] as Float)
  }
  ...
  updates.systemEvents.filter { it["type"] == "midi-export" }.forEach {
    awaitActiveTasks()
    engine().export(it["filepath"] as String)
  }
  ...
  updates.systemEvents.filter { it["type"] == "shutdown" }.forEach {
    engine().scheduleShutdown(it["offset"] as Int)
  }
  ...
  if (updates.systemActions.contains(SystemAction.PLAY) && !engine().isPlaying) {
    awaitActiveTasks()
